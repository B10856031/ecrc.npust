...
for ($i = 0; $i < $_POST['count']; $i++) {
	if (file_exists($_FILES['file']['tmp_name'][$i])) {
		...
		// for ($i = 0; $i < $_POST['count']; $i++) {
			if (move_uploaded_file($_FILES['file']['tmp_name'][$i],$img_folder.'/' .  $_FILES['file']['name'][$i])) {
				// echo "yes";
				$result="yes";
				// echo  "imgfolder ".$img_folder;
				// echo "~".$_FILES['file']['type'][$i].'*';
			} else {
				// echo "檔案搬移失敗，請確認{$_POST['save_path']}資料夾可寫入" . $_FILES['file']['tmp_name'][$i] . $_FILES['file']['name'][$i];
			}
		// }
